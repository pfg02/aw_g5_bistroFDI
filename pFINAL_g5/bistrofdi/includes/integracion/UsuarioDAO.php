...
class UsuarioDAO
{
    ...
    public function insertar(UsuarioDTO $usuario): bool
    {
        $stmt = $this->conn->prepare(
            'INSERT INTO usuarios (nombre_usuario, email, nombre, apellidos, password_hash, rol, avatar, vip)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        );
		...
		$vip = $usuario->getVip() ? 1 : 0;

        $stmt->bind_param('sssssss', $nombreUsuario, $email, $nombre, $apellidos, $passwordHash, $rol, $avatar, $vip);
        $ok = $stmt->execute();
        $stmt->close();
    ...
    {
        $stmt = $this->conn->prepare(
            'UPDATE usuarios SET email = ?, nombre = ?, apellidos = ?, rol = ?, avatar = ?, vip = ? WHERE id = ?'
        );
        ...
        $stmt->bind_param('sssssi', $email, $nombre, $apellidos, $rol, $avatar, $vip, $id);
        $ok = $stmt->execute();
        $stmt->close();
