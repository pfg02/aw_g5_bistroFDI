...
class ProductoDAO
{
    ...
    private function mapear(array $row): ProductoDTO
    {
        return new ProductoDTO(
			...
			null, // El nombre de categoría se asigna en listarTodos() y listarOfertados() con el alias cat_nombre.
            $row['ingredientes'] !== null ? (string) $row['ingredientes'] : ''
        );
    }
    ...
    public function guardar(ProductoDTO $p): bool
    {
        ...
        $ingredientes = (string) $this->obtenerDatoProducto($p, 'ingredientes');
        if ($id !== null) {
            $sql = "UPDATE productos
                    SET nombre = ?, descripcion = ?, precio_base = ?, stock = ?, imagen = ?, id_categoria = ?, ofertado = ?, requiere_cocina = ?, iva = ?, ingredientes = ?
                    WHERE id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->bind_param(
                'ssdisiiiisi',
            ...
            $sql = "INSERT INTO productos
                    (nombre, descripcion, precio_base, stock, imagen, id_categoria, ofertado, requiere_cocina, iva, ingredientes)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $this->db->prepare($sql);
            $stmt->bind_param(
                'ssdisiiiiis',
     ...
     * Devuelve 'anadido' o 'quitado' para que el JS sepa qué color poner.
     */
