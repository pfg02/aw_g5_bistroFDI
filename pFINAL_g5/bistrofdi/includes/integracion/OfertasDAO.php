...
class OfertaDAO
{
    ...
    {
        $sql = "SELECT id, nombre, descripcion, fecha_inicio, fecha_fin, descuento_porcentaje, solo_vip
                FROM ofertas
                WHERE activa = 1
    ...
    public function crearOferta(OfertaDTO $oferta): bool
    {
        ...
        try {
            $sql = "INSERT INTO ofertas
                    (nombre, descripcion, fecha_inicio, fecha_fin, descuento_porcentaje, activa, solo_vip)
                    VALUES (?, ?, ?, ?, ?, 1, ?, ?)";
    ...
    private function mapearOferta(array $row): OfertaDTO
    {
        ...
        $oferta->setProductos([]);
		$oferta->setSoloVip((bool) $row['solo_vip']);
