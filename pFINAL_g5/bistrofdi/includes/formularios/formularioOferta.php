...
class FormularioOferta extends Formulario
{
    ...
    protected function generaCamposFormulario(array $datos): string
    {
		...
		if (!empty($datos)) {
			// Si se ha enviado, miramos si el checkbox venía marcado
			$esVIP = isset($datos['vip']) ? 'checked' : '';
		} else {
			// Si es la primera vez que entramos (editar), leemos la base de datos
			$esVIP = $this->obtenerDatoOferta('solo_vip') ? 'checked' : '';
		}
...
<div class="grupo-control">
    <label for="descripcion">Oferta VIP</label>
    <input type="checkbox" name="vip" id="vip" value="1" $esVIP>
</div>
...
HTML;
    ...
    protected function procesaFormulario(array $datos): ?string
    {
		...
		$esVip = isset($datos['vip']) && $datos['vip'] === '1' ? 1 : 0;
